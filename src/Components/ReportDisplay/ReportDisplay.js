...
export default class ReportDisplay extends Component {
  ...
  findUniqueReport = () => {
    ...
    const srcResult = this.context.reports.filter(
      (report) => report.id === srcId
    );
    if (srcResult.length === 1)
      this.setState({
        uniqueReport: srcResult[0],
        oldReports: this.context.reports,
      });
  };
      ...
      if (!this.state.editMode) {
            ...
            <section>
              ...
              <div className={"report-details"}>
                <table>
                  ...
                  <tr>
                    <td>Latitude</td>
                    <td>{parseInt(rep.report_lat)}</td>
                  </tr>
                  <tr>
                    <td>Longitude</td>
                    <td>{parseInt(rep.report_lng, 10)}</td>
                  </tr>
                  <tr>
                  ...
                  <tr>
                    ...
                    {rep.report_waterbody ? (
                      <td>{rep.report_waterbody}</td>
                    ) : (
                      <td>{`no body of water given`}</td>
                    )}
                  </tr>
                  <tr>
                    <td>extra details, if any</td>
                    {rep.report_details !== "undefined" ? (
                      <td>{rep.report_details}</td>
                    ) : (
                      <td>{"no extra details given"}</td>
                    )}
                  </tr>
                </table>
              </div>

              <button onClick={() => this.handleDelete(rep.id)}>
                Resolve (delete)
              </button>
        ...
        return (
          <div>
            ...
            <Form
              defaultVals={this.state.uniqueReport}
